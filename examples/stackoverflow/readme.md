...
This article will introduce a tool that extracts JSON from HTML. We will use the HTML from StackOverflow to illustrate how to use temme. Temme is based on [cheerio](https://cheerio.js.org/) and uses CSS selectors extensively, so [MDN](https://developer.mozilla.org/zh-CN/docs/Learn/CSS/Introduction_to_CSS/Selectors) may be helpful.
...
Recent days, I spend a lot of time on Node web crawlers. I use [cheerio](https://github.com/cheeriojs/cheerio) (server-side jQuery) to parse HTML. Cheerio implements various functions from core jQuery and provides a lot APIs that query/edit/delete/add nodes to manipulate the HTML document. However, as more and more HTML documents are crawled, the code that uses cheerio to parse HTML becomes complex and intractable. Web crawlers have some common patterns to extract data from HTML which do not fit well with cheerio. Here are three observations:

![StackOverflow Index Page](./imgs/top-questions.jpg)

The following examples assume that we are crawling a list of top questions from StackOverflow as the above figure shows.

1. Different items need to be extracted on one node. For example, every question has a title, a link, a list of tags, and counts for votes/answers/views. All these items is stored in a node that matches `div.question-summary`.
...
Temme is a tool that I have developed to extract JSON from HTML. Temme adds some extra grammar on CSS selector. The methods that temme adopts to deal with these patterns are:
...
After getting a list of top questions, we can follow the link of each question to get the detail of the question. Here, we select [this question](https://stackoverflow.com/questions/47772015/toggle-between-two-stylesheets) as our exmaple as the above image shows. We use the following temme-selector to extract various information about the question:

![Question Detail Selector](./imgs/question-detail-selector.jpg)

The temme-selector is so long that we need to save it to a file namely *question-detail.temme*. We then use the filename to refer to the selector.
...
There are some strange syntax constructs and their meanings are as follows: The `&` (line 9, 11) means referrence to the parent node (like in less, sass or [stylus](http://stylus-lang.com/docs/selectors.html#parent-reference)); In line 10, temme first sets `$accept` to `false`, then if temme finds that the parent node matches `.accepted-answer` (line 11), temme sets `$accept` to `true`.

When implementing a web crawler, we first analyze the structure of the web page. Then write down the temme-selector for every kind of page on the playground and then save the selector to a file. When the crawler have downloaded the HTML document, we use the corresponding selector to extract JSON from HTML.

## Conclusion

The examples have covered most of the temme. In practice, most websites have a clear HTML structure which is easy to analyze, and temme fits well in such situations. Temme turns out to be a very good tool to extract structured JSON from HTML. You can goto [the GitHub repository](https://github.com/shinima/temme) for the full documentation.
